...
- **优化（最终阶段）**：依赖所有期望的用户故事完成

### 用户故事依赖

- **用户故事 1（P1）**：基础（阶段 2）完成后可开始 - 不依赖其他故事
- **用户故事 2（P2）**：基础（阶段 2）完成后可开始 - 可能与 US1 集成但应可独立测试
- **用户故事 3（P3）**：基础（阶段 2）完成后可开始 - 可能与 US1/US2 集成但应可独立测试

### 每个用户故事内部

- 测试（如包含）必须先编写并失败，然后再实现
- 模型在服务之前
- 服务在端点之前
- 核心实现在集成之前
- 故事完成后再进入下一优先级
...
- 所有标记 [P] 的设置任务可并行执行
- 所有标记 [P] 的基础任务可并行执行（在阶段 2 内）
- 基础阶段完成后，所有用户故事可并行开始（如团队有能力）
- 一个用户故事的所有标记 [P] 的测试可并行执行
- 故事内标记 [P] 的模型可并行执行
- 不同用户故事可由不同团队成员并行处理
