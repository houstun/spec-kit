...
1. 从仓库根目录运行 `{SCRIPT}`，解析 FEATURE_DIR 和 AVAILABLE_DOCS 列表。所有路径必须是绝对路径。对于参数中的单引号，如 "I'm Groot"，使用转义语法：例如 'I'\''m Groot'（或尽可能使用双引号："I'm Groot"）。
...
2. **检查清单状态**（如果 FEATURE_DIR/checklists/ 存在）：
   ...
   - **如果有任何清单未完成**：
     - 显示带有未完成项目数的表格
     - **停止**并询问："部分清单尚未完成。是否仍要继续实施？（是/否）"
     - 等待用户响应后再继续
     - 如果用户回答"否"或"等待"或"停止"，则停止执行
     - 如果用户回答"是"或"继续"或"proceed"，则进入步骤 3
...
6. 按任务计划执行实施：
   ...
   - **遵守依赖关系**：按顺序运行顺序任务，带 [P] 标记的并行任务可以同时运行
   - **遵循 TDD 方法**：在对应的实现任务之前执行测试任务
   - **基于文件的协调**：影响相同文件的任务必须顺序运行
   - **验证检查点**：在继续之前验证每个阶段是否完成
